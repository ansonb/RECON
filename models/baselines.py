...
class ContextAware(nn.Module):
    ...
    def forward(self, sentence_input, entity_markers):
        ...
        for i in range(self.MAX_EDGES_PER_GRAPH):
            ...
            elif i == self.MAX_EDGES_PER_GRAPH - 1:
            ...
            sentence_scores = torch.matmul(target_sentence_memory.unsqueeze(
                1).unsqueeze(1), context_vectors.unsqueeze(3)).squeeze(dim=-1).squeeze(dim=-1)
        ...
        edge_vectors = edge_vectors.view(sentence_input.size()[0] * self.MAX_EDGES_PER_GRAPH, -1)
