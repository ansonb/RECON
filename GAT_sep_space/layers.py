...
class ConvKB(nn.Module):
    def __init__(self, input_dim, input_seq_len, in_channels, out_channels, drop_prob, alpha_leaky):
        ...
        self.conv_layer = nn.Conv2d(
            in_channels, out_channels, (1, input_seq_len))  # kernel size -> 1*input_seq_length(i.e. 3)
    ...
    def forward(self, conv_input):
        ...
        h1 = self.nl1(self.fc1(conv_input))
        output = self.fc2(h1)
...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, edge, edge_embed, edge_list_nhop, edge_embed_nhop):
        ...
        # print('== edge_embed')
        # print(edge_embed)
        # assert not torch.isnan(edge_embed).any()
        # print('== edge_embed_nhop')
        # print(edge_embed_nhop)
        # assert not torch.isnan(edge_embed_nhop).any()

        # assert not torch.isnan(edge).any()
        # assert not torch.isnan(edge_list_nhop).any()
        ...
        # to be checked later
        powers = -self.leakyrelu(self.a_2.mm(edge_m).squeeze(0))
        # print('== powers')
        # print(powers)
