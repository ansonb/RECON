...
def build_data(path='./data/WN18RR/', is_unweigted=False, directed=True):
    ...
    test_triples, _, unique_entities_test = load_data(os.path.join(
        path, 'test.txt'), entity2id, relation2id, is_unweigted, directed)
    _, test_adjacency_mat, _ = load_data(os.path.join(
        path, 'test.txt'), entity2id, relation2id, is_unweigted, directed)
    ...
    for i in range(len(relation2id)):
        if left_entity.get(i,None) is not None:
    ...
    right_entity_avg = {}
    for i in range(len(relation2id)):
        if right_entity.get(i,None) is not None:
    ...
    headTailSelector = {}
    for i in range(len(relation2id)):
        if right_entity_avg[i] + left_entity_avg[i] > 0:
    ...
    return (train_triples, train_adjacency_mat), (validation_triples, valid_adjacency_mat), (test_triples, test_adjacency_mat), \
        entity2id, relation2id, headTailSelector, unique_entities_train, unique_entities_test
